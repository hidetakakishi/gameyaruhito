  ...
  <div id="chat-info-modal" class="relative z-10 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 z-10">
      ...
      <div class="flex min-h-full justify-center p-4 text-center items-center sm:p-0">
          <div class="lg:w-1/3 md:w-1/2 bg-white rounded-lg p-8 flex flex-col w-full mt-10 md:mt-0 relative z-10 shadow-md justify-center items-center break-all">

                <h2 class="text-gray-900 text-lg mb-6 font-bold w-10/12">{{ $chat->title }}</h2>
                <p class="leading-relaxed text-gray-600 mb-1">ホスト</p>
                <div class="flex justify-center items-center">
                    <img class="h-8 w-8 rounded-full" src="{{ $chat->avatar }}" alt="プロフィール">
                    <span class="text-gray-600 dark:text-gray-300 text-sm float-left ml-2">{{$chat->name}}<br><span class="text-sm text-gray-400 float-left">{{ '@' . $chat->twitter_screen_name }}</span></span>
                </div>
                @if(1 < count($chatMemberList))
                <p class="leading-relaxed text-gray-600 mb-1 mt-3">メンバー</p>
                @foreach($chatMemberList as $user)
                @if($user->twitter_id != $chat->twitter_id)
                <div class="flex justify-center items-center">
                    <img class="h-8 w-8 rounded-full" src="{{ $user->avatar }}" alt="プロフィール">
                    <span class="text-gray-600 dark:text-gray-300 text-sm float-left ml-2">{{$user->name}}<br><span class="text-sm text-gray-400 float-left">{{ '@' . $user->twitter_screen_name }}</span></span>
                </div>
                @endif
                @endforeach
                @endif
                <p class="leading-relaxed text-xs text-gray-500 mt-8">募集人数{{ $userCount - 1 . '/' . $chat->capacity }}人</p>
                @if($chat->followers_only_flag == true)
                  <p class="leading-relaxed text-xs text-gray-500">条件:フォロワーのみ</p>
                @elseif($chat->mutual_followers_only_flag == true)
                  <p class="leading-relaxed text-xs text-gray-500">条件:相互フォロワーのみ</p>
                @else
                  <p class="leading-relaxed text-xs text-gray-500">条件:なし</p>
                @endif

                <a href="https://twitter.com/share?url={{ route('chat.preview', $chat->unique_key) }}" rel="nofollow noopener" target="_blank" class="text-xs">
                  <div class="flex rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 absolute left-5 top-5">
                    <img class="h-7 w-7 rounded-full bg-white border-white" src="{{ asset('image/twitter.jpeg') }}" alt="プロフィール">
                  </div>
                </a>
                ...
                <p class="text-xs text-gray-500 mt-3">
                  @php
                      ...
                      echo 'このつのりたは残り' . $diff->format('%h時間%i分') . 'で終了します';
                  @endphp
                </p>
                ...
                <button type="button" class="py-1 px-1 bg-red-600 hover:bg-red-700 focus:ring-red-600 focus:ring-offset-red-200 text-white w-full transition ease-in duration-200 text-center text-xs font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-full mt-4 w-28 delete-event">
                つのりたを消す
                </button>
                @else
                ...
                <button type="button" class="py-1 px-1 bg-red-600 hover:bg-red-700 focus:ring-red-600 focus:ring-offset-red-200 text-white w-full transition ease-in duration-200 text-center text-xs font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-full mt-4 w-24 leaving-event">
                退出する
                </button>                
                @endif
